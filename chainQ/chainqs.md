...
17. 在区块链底层，数据通常以区块的形式存储。请描述一个区块中的交易数据是如何在存储系统（比如 LevelDB）中组织和存储的？包括数据结构和存储的顺序、索引等方面。
18. 区块链数据随着时间推移会不断增长，这对存储设备的容量和性能有很高要求。在设计区块链底层存储方案时，如何考虑数据的归档和清理？以比特币区块链为例，阐述其可能的归档策略和清理过期数据的方法。
19. 在区块链底层，如何防止交易的重复提交？比如在高并发情况下，同一用户可能多次尝试提交相同交易，阐述所使用的技术手段和原理。
20. 当区块链底层收到一笔新的交易时，从接收到验证再到将其放入交易池等待打包的整个流程是怎样的？请以以太坊区块链为例，详细描述其中涉及的步骤和检查机制。
